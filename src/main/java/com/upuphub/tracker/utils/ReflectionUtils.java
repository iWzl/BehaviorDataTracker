...
import com.upuphub.tracker.exception.CanNotInvokeTargetMethodException;
import com.upuphub.tracker.exception.CheckClassScanParamsException;
...
public final class ReflectionUtils {
     ...
     * MAP的初始化数组大小
     */
    private static final Integer MAP_INIT_SIZE = 32;
    ...
    public static Set<Class<?>> scanAnnotatedClass(String[] packageNames, Class<? extends Annotation> annotation) {
        ...
        if (LOGGER.isTraceEnabled()){
            LOGGER.trace("The number of class Annotated with  @{} :[{}]",
                    annotation.getSimpleName(), annotatedClass.size());
        }
     ...
     * @return the classes marked by the specified annotation in the specified package
     */
    public static Map<Class<? extends Annotation>,
            Set<Class<?>>> scanAnnotatedClasses(String[] packageNames,
                                                Set<Class<? extends Annotation>> annotationList) {
        ...
        Reflections reflections = new Reflections(packageNames, new TypeAnnotationsScanner());
        Map<Class<? extends Annotation>, Set<Class<?>>> annotatedClassesMap =
                new ConcurrentHashMap<>(annotationList.size());
    ...
    public static Field[] getFields(Class<?> clazz, Predicate<Field> fieldFilter) {
        List<Field> fields = new ArrayList<>(MAP_INIT_SIZE);
